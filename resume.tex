...
% \hfill) and bullet text never runs into the date column.
\newlength{\jobdateswidth}
\setlength{\jobdateswidth}{0.9in}
\newlength{\jobheadergap}
\setlength{\jobheadergap}{0.3in}
% Narrowed below the width the header/date math alone would give, so
% bullet lines stay short and easy to read. The header tabular sizes to
% its own contents (title + gap + date), so narrowing \jobtitlewidth
% pulls the date column in too -- keeping the header and the bullets
% below it flush with the same right edge instead of leaving a ragged
% gap of unused space next to short bullet lines.
...
\addtolength{\jobtitlewidth}{-0.4in}
